...
unsigned long notifyAccumulator = 0;

/**
  Setting global variables to track the last time ellapsed time data and notify accumulator are emitted.
  I intend to emit the ellapsedTime value every second, and the notify accumulator value every 5 seconds
*/
unsigned long lastEllapsedTimestamp = 0;
...
void loop() {
  ...
  // After every 1 second, update ellapsed time value
  if (current - lastEllapsedTimestamp >= 1000) {
    lastEllapsedTimestamp = current;
  ...
  // after every 5 seconds, update and broadcast the notify accumulator
  if (current - lastNotifyTimestamp >= 5000) {
    lastNotifyTimestamp = current;
